sleep_dat <- read.csv("/Users/Andrew/Desktop/UCL Stuff/UCLDSSC/PEACHData/BigData/PEACH_sleepQualityLog_2Q2015.csv")
fd_dat <- read.csv("/Users/Andrew/Desktop/UCL Stuff/UCLDSSC/PEACHData/BigData/PEACH_foodDiary_2Q2015.csv")
fd_all_dat <- read.csv("/Users/Andrew/Desktop/UCL Stuff/UCLDSSC/PEACHData/BigData/PEACH_allergyConsumption_2Q2015.csv")
fd_sent_dat <- read.csv("/Users/Andrew/Desktop/UCL Stuff/UCLDSSC/PEACHData/BigData/PEACH_sentimentFeedback_2Q2015.csv")
af_dat <- read.csv("/Users/Andrew/Desktop/UCL Stuff/UCLDSSC/PEACHData/BigData/PEACH_atmosphericFactorsLog_2Q2015.csv")
...
allergen_dat <- matrix(nrow = nrow(sleep_dat_sub), ncol = 13)
allergen_dat[which(dinner_dat_sub$Dinner_Apple > 0),1] <- dinner_dat_sub$Dinner_Apple[dinner_dat_sub$Dinner_Apple > 0]
...
# save(sleep_data_full, file = "fixed_sleep_data_full.rda")
...
glmnet.mod <- glmnet(x = as.matrix(train_set[,c(2,4,6,8:54)]), y = train_set$Sleep_Quality_Value,
                     family = "gaussian", alpha = 0, lambda = seq(0.01, 0.1, by = 0.01))
preds1 <- predict(glmnet.mod, as.matrix(validation_set[,c(2,4,6,8:54)]))
errors <- c()
for (i in 1:10) {
  errors[i] <- mean(abs(validation_set$Sleep_Quality_Value - preds1[,i]))
}
